...
## 11. Tweet the headline change

After the checker pass survives the gate and the PR is open, draft **one** tweet from the month's CHANGELOG entry and put it in the PR body under a **"Tweetable"** heading, with the chart to attach. Never post it automatically — Nathan reviews the tweet with the data and posts it himself after merge (the chart must be live on stateof.ai first).

**Picking the change (materiality order):**
1. `demand` moves — GW commitments and vendor diversification (NVIDIA vs non-NVIDIA share). These are the strongest narratives.
2. First-of-a-kind operators — a new industry (pharma, trading), country, or operator class entering a fleet chart.
3. Large tier moves to Deployed (announced pipelines becoming live capacity at scale).
4. Additions that only grow an existing bar, label changes, and re-sorts are **not** tweet-worthy on their own; if nothing clears the bar, write "Tweetable: nothing this month" in the PR body rather than forcing one.

**Voice (Nathan's):** lead with what changed and the exact numbers; one plain-English clause on why it matters at most; British English; no hype words ("huge", "insane"), no rocket emoji, no hashtags; ≤280 chars counting the URL as 23 (t.co); end with `stateof.ai/compute`. Shape: `<what refreshed>. <the change, numbers first>. <second data point if room>. stateof.ai/compute`.

**Chart to attach:** name the slug and give both options — attach `og/<slug>.png` (freshly regenerated by `render_og.sh`, renders larger on X) or paste `stateof.ai/compute/share/<slug>` for the auto-unfurled card. If a previously shared card image changed, remind about the card-validator re-scrape (§5 step 9).
